...
export function AQIDashboard() {
  ...
  clientId: 'web_dashboard_' + Math.random().toString(16).substring(2, 8), // Unique ID is required
  protocol: 'wss',
  path: '/mqtt',      // HiveMQ Cloud WebSockets usually need this path
  clean: true,
  connectTimeout: 10000, // Increase to 10s for slow TLS handshakes
  ...
  // Calculate respiratory risk factor (0-100)
  const calculateRiskFactor = (): { score: number; label: string; color: string } => {
    ...
    // CO2: 40% weight (ventilation quality)
    if (metrics.co2 !== null) {
  ...
  const getMetricStatus = (metric: string, value: number | null): { color: string; icon: React.ReactNode; alert: boolean } => {
    ...
    // PM1 thresholds (ultra-fine particles, most harmful)
    if (metric === 'pm1') {
    ...
    // PM2.5 thresholds (WHO strict guideline for vulnerable lungs)
    if (metric === 'pm25') {
    ...
    // CO2 thresholds (poor ventilation & viral stagnation risk)
    if (metric === 'co2') {
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className={`bg-slate-900 border-slate-700 transition-all ${pm1Status.alert ? 'border-red-500 shadow-lg shadow-red-500/20 animate-pulse' : ''}`}>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              ...
              PM1
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">µg/m³</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className={`text-3xl font-bold ${pm1Status.color}`}>{metrics.pm1 ?? '—'}</div>
        ...
        <Card className={`bg-slate-900 border-slate-700 transition-all ${pm25Status.alert ? 'border-red-500 shadow-lg shadow-red-500/20 animate-pulse' : ''}`}>
          ...
          <CardContent>
            <div className={`text-3xl font-bold ${pm25Status.color}`}>{metrics.pm25 ?? '—'}</div>
            <p className="text-xs text-slate-400 mt-1">
              {metrics.pm25 !== null ? (metrics.pm25 > 35 ? '⚠️ EXCEEDS THRESHOLD' : metrics.pm25 > 30 ? 'Caution' : 'Safe') : 'No data'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className={`bg-slate-900 border-slate-700 transition-all ${co2Status.alert ? 'border-red-500 shadow-lg shadow-red-500/20 animate-pulse' : ''}`}>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              ...
              CO₂
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">ppm</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className={`text-3xl font-bold ${co2Status.color}`}>{metrics.co2 ?? '—'}</div>
            <p className="text-xs text-slate-400 mt-1">
              {metrics.co2 !== null ? (metrics.co2 > 800 ? '⚠️ POOR VENTILATION' : metrics.co2 > 600 ? 'Elevated' : 'Good') : 'No data'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className={`bg-slate-900 border-slate-700 transition-all ${ozoneStatus.alert ? 'border-amber-500 shadow-lg shadow-amber-500/20' : ''}`}>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              ...
              Ozone
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">ppb</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className={`text-3xl font-bold ${ozoneStatus.color}`}>{metrics.ozone ?? '—'}</div>
            <p className="text-xs text-slate-400 mt-1">
        ...
        <Card className="bg-slate-900 border-slate-700">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-slate-300">{metrics.pm10 ?? '—'}</div>
            <p className="text-xs text-slate-400 mt-1">Particulate matter</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-700">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-slate-300">{metrics.tvoc ?? '—'}</div>
            <p className="text-xs text-slate-400 mt-1">Volatile organics</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              Temperature
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">°C</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-slate-300">{metrics.temperature !== null ? metrics.temperature.toFixed(1) : '—'}</div>
        ...
        <Card className="bg-slate-900 border-slate-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              Humidity
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">%</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-slate-300">{metrics.humidity !== null ? metrics.humidity.toFixed(1) : '—'}</div>
            <p className="text-xs text-slate-400 mt-1">Relative humidity</p>
          </CardContent>
        </Card>
<Card className="bg-slate-900 border-slate-700">
  <CardHeader className="pb-2">
      ...
      Atmospheric pressure
        ...
        <Card className="bg-slate-900 border-slate-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-300 flex items-center gap-2">
              AQI
              <span className="text-xs bg-slate-800 px-2 py-1 rounded text-slate-400">Index</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-slate-300">{metrics.aqi ?? '—'}</div>
